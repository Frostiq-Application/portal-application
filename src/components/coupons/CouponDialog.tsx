...
export function CouponDialog({ open, onOpenChange, coupon }: Props) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setMaxDiscount(coupon?.maxDiscountAmount ? String(Number(coupon.maxDiscountAmount)) : "");
      setMinOrder(coupon?.minOrderAmount ? String(Number(coupon.minOrderAmount)) : "");
      setUsageTotal(coupon?.usageLimitTotal != null ? String(coupon.usageLimitTotal) : "");
      setUsageUnlimited(coupon?.usageLimitTotal == null);
      setUsagePerCustomer(coupon?.usageLimitPerCustomer != null ? String(coupon.usageLimitPerCustomer) : "");
  ...
  const submit = async () => {
    ...
    if (!validFrom || !validUntil) return toast.error("Validity window is required");
    ...
    const common = {
      ...
      usageLimitPerCustomer: usagePerCustomer === "" ? undefined : Number(usagePerCustomer),
    ...
    try {
      if (isEdit && coupon) {
        ...
        const body: CreateCouponBody = { shopId, code: code.trim().toUpperCase(), ...common };
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-2 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5">
            <Label>Code</Label>
            <Input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="FLAT100" disabled={isEdit} />
          </div>
          <div className="flex flex-col gap-1.5">
            <Label>Type</Label>
            <Select value={discountType} onValueChange={(v) => setDiscountType(v as CouponType)}>
          ...
          <div className="flex flex-col gap-1.5">
            <Label>{discountType === "percentage" ? "Percent" : "Amount (₹)"}</Label>
            <Input type="number" min={0} value={discountValue} onChange={(e) => setDiscountValue(e.target.value)} />
          </div>
          {discountType === "percentage" && (
            <div className="flex flex-col gap-1.5">
              <Label>Max discount (₹)</Label>
              <Input type="number" min={0} value={maxDiscount} onChange={(e) => setMaxDiscount(e.target.value)} placeholder="No cap" />
            </div>
          )}
          <div className="flex flex-col gap-1.5">
            <Label>Min order (₹)</Label>
            <Input type="number" min={0} value={minOrder} onChange={(e) => setMinOrder(e.target.value)} placeholder="None" />
          </div>
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              <Label>Total usage limit</Label>
              <div className="flex items-center gap-2">
                <Switch id="usageUnlimited" checked={usageUnlimited} onCheckedChange={setUsageUnlimited} />
                <Label htmlFor="usageUnlimited" className="cursor-pointer text-xs font-normal text-muted-foreground">Unlimited</Label>
              </div>
            </div>
            {usageUnlimited ? (
              <Input value="Unlimited" disabled />
            ) : (
              <Input type="number" min={1} value={usageTotal} onChange={(e) => setUsageTotal(e.target.value)} placeholder="Enter count" />
            )}
          </div>
          <div className="flex flex-col gap-1.5">
            <Label>Per-customer limit</Label>
            <Input type="number" min={1} value={usagePerCustomer} onChange={(e) => setUsagePerCustomer(e.target.value)} placeholder="Unlimited" />
          </div>
          <div className="flex flex-col gap-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="pub" className="cursor-pointer">Public banner</Label>
          </div>
          <div className="flex flex-col gap-1.5">
            <Label>Display label</Label>
            <Input value={displayLabel} onChange={(e) => setDisplayLabel(e.target.value)} placeholder="Flat ₹100 OFF" />
          </div>
        </div>
