...
import type { LoginResponse, OrderEvent } from "@/types";
 ...
 * change. Uses fetch-event-source (not native EventSource) so it can send the
 * Bearer token as a header and transparently refresh it on 401 — mirroring the
 * RTK Query baseApi reauth flow.
 *
...
export function useOrderStream(onEvent: (e: OrderEvent) => void): StreamStatus {
  ...
  useEffect(() => {
    ...
    fetchEventSource(`${API_BASE_URL}/orders/stream`, {
      ...
      async onopen(res) {
        ...
        // 403 (platform admin) or other non-retriable statuses: stop.
      ...
      onmessage(msg) {
        if (!msg.data) return;
        try {
          onEventRef.current(JSON.parse(msg.data) as OrderEvent);
        } catch {
